...
const fetchPokemons = async() => {
    const queryOffset = document.querySelectorAll(".pokemon__card").length + 1;
...
function createCard(pokemon){
    ...
    card.classList.add("pokemon__card", "rounded", "bg__darkgray", "fluid");
    ...
    cardCover.classList.add("pokemon__card__cover", "fluid", "bg__lightblack");
    cardCover.style.backgroundImage = `url(${image})`;
    
    const cardInfo = document.createElement("div");
    cardInfo.classList.add("pokemon__card__info");

    const pokemonName = document.createElement("div");
    pokemonName.classList.add("pokemon__card__info__name", "text__color__gray");
    pokemonName.innerText = pokemon.name;
    cardInfo.appendChild(pokemonName);

    const typeContainer = document.createElement("div");
    typeContainer.classList.add("pokemon__type__container");

    pokemonTypes.forEach(pokemonType => {
        const typeElement = document.createElement("div");
        typeElement.classList.add("pokemon__type", "rounded", `${pokemonType.type.name}`, "bg__lightblack");
    ...
    document.querySelector("#pokemon__grid").appendChild(card);
}